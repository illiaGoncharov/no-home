...
function addImportMap(content) {
    var importMapMatch = content.match(/<script\s+type=["']importmap["']>([\s\S]*?)<\/script>/i);
    if (importMapMatch) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                Promise.all(scriptPromises).then(() => {
                    const initFunction = clickedClasses.find(cls => initializeFunctions.hasOwnProperty(cls));
...
/////////////////////// Перемещение пультика и закрытие пультика
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const displayButtonWrapper = document.querySelector('.display-button-wrapper');
            ...
            function loadSavedState() {
                ...
                if (savedState) {
                    ...
                    displayButtonWrapper.style.display = savedState.displayButtonWrapperDisplay;
                }
            }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!elements.buttonContainer || !elements.button || !elements.home || !elements.homeLink || !elements.buttonImg || !elements.homeLinkImg) {
        return;
    }
    ...
    function toggleSkeletonHome(show) {
        if (show) {
            ...
            elements.home.addEventListener('transitionend', function hideHome() {
...
function updateValue() {
    ...
    const percent = (value - volumeRange.min) / (volumeRange.max - volumeRange.min);
    ...
    function updateRedSquarePosition() {
        ...
        const thumbPosition = (volumeRange.value - volumeRange.min) / (volumeRange.max - volumeRange.min);
...
const nonFluidRooms = ['window-in-table-room', 'outside-in-bedroom-room', 'just-cave'];
