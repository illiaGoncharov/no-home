...
function initializeIphoneScript() {
  ...
  document
    .querySelector(".apps-screen")
    .addEventListener("click", playAllVideos);
  ...
  function playNextTrack() {
    ...
    if (currentTrack) {
      ...
      const nextTrack = tracks.find(
        (track) => track.dataset.order === String(nextOrder)
      );
  ...
  function playPreviousTrack() {
    ...
    if (currentTrack) {
      ...
      const prevTrack = tracks.find(
        (track) => track.dataset.order === String(prevOrder)
      );
